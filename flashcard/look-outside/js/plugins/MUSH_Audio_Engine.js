...
@ ------------------------------------------------
*
* @command AddSpacialBgsVar
* @text Add Spacial BGS from Var
* @desc Add a non-dynamic spacial BGS.
*
* @arg Filename
* @text Filename
* @desc Select the BGS filename var.
* @type variable
* @default 1
*
* @arg Pitch
...
* @desc Play a BGS from a variable
* 
* @arg Filename
* @text Filename
* @desc Select the Variable for BGS to play.
* @type variable
*
* @arg Channel
* @text Channel
...
PluginManager.registerCommand('MUSH_Audio_Engine', 'AddSpacialBgsVar', args => {
	var r = Number(args.Radius);
	if (r == 0) {
		var rRadius = Math.sqrt(Math.pow($dataMap.width / 2, 2) + Math.pow($dataMap.height / 2, 2));
	} else {
	...
	var getFile = $gameVariables.value(args.Filename);
	var getPitch = $gameVariables.value(args.Pitch);
	var obj = AudioManager.createSpacialAudioObject(String(getFile), Number(getPitch), Number(args.Channel), 
				'bgs', eval(args.Dynamic), Number(args.MaxVolume), rRadius, Number(args.Strength), 
				String(args.Pan), Number(args.PanSt), Number(args.PanLd));
	$gamePlayer.addSpacialBGS(obj);
	var bgs = {name: obj.filename, pitch: obj.pitch, volume: 0};
...
PluginManager.registerCommand('MUSH_Audio_Engine', 'PlayBgsVar', args => {
	var getFile = $gameVariables.value(args.Filename);
	var bgs = {name: String(getFile), pitch: Number(args.Pitch), volume: Number(args.Volume), pan: Number(args.Pan)};
	if (!AudioManager.checkAlreadyHaveBgsAudioChannel(args.Channel)) {
		AudioManager.playMushBgs(bgs, Number(args.Channel), eval(args.AutoRemover), args.Interrupt);
		if (Number(args.FadeIn) > 0) {
			AudioManager.fadeInMushBgs(Number(args.Channel), Number(args.FadeIn));
