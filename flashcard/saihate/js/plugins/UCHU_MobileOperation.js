//2021_12_31 修正済み
//2023_11_20 仮想パッドの高さを調整
//2024_02_16 iPadの表示位置修正
...
(function() {
	...
	PRM.sensitivity = Number(Parameters["Analog Sensitivity"]);
        ...
        var dd = window.navigator.userAgent.toLowerCase();
        if(dd.indexOf('ipad') > -1 
        || dd.indexOf('macintosh') > -1 && 'ontouchend' in document){
        var onipad = 1;
...
{if (onipad == 1){
...
else{PRM.visible[0]=PRM.visible[1]=PRM.visible[2]=false;}
}
	//-----------------------------------------------------------------------------
	// Locate_DirPad
	...
	Scene_Base.prototype.start = function() {
	    ...
	    if (onipad == 1 || Utils.isMobileDevice() || PRM.pcBtn) {
			if(!Btn_ready){
				Btn_ready=true;
	...
	if(PRM.visible[0] || PRM.visible[1] || PRM.visible[2]){
		...
		Graphics._updateRealScale = function() {
			...
			if (onipad == 1 || this._stretchEnabled) {
	...
	if(PRM.hideBtn){
		Scene_Base.prototype.hideUserInterface = function() {
			if (onipad == 1 || Utils.isMobileDevice() || PRM.pcBtn) {Btn_hide=true;
		...
		Scene_Base.prototype.showUserInterface = function() {
			if (onipad == 1 || Utils.isMobileDevice() && !Btn_hide || PRM.pcBtn && !Btn_hide) {
	...
	if(onipad == 1 || Utils.isMobileDevice() || PRM.pcExt){
		if(PRM.holdaction){
		...
		if(PRM.flickpage || PRM.outcansel || PRM.outaction){
			...
			if(onipad == 1 || Utils.isMobileDevice()){
	...
	//AnalogMove.js
	if(PRM.analogmove && onipad == 1 || Utils.isMobileDevice() || PRM.analogmove && PRM.pcBtn){
