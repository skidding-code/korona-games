...
Graphics._createEffekseerContext = function() {
    if (this._app && window.effekseer) {
        // Canvas2D fallback has no GL context — Effekseer requires WebGL
        const gl = this._app.renderer.gl;
        if (!gl) {
            console.warn("Effekseer disabled: no WebGL context (Canvas2D fallback active).");
            this._effekseer = null;
            return;
        }
        try {
            ...
            console.warn("Effekseer init failed, effects disabled:", e);
            this._effekseer = null;
            // Do NOT null out this._app — game still runs without effects
        }
    }
};
